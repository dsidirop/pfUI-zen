...
function linq.lambda(expr)
    ...
    if args and rets then
        ...
        if newrets == rets then
            rets = rets:sub(2, table.getn(rets) - 1)
        end
...
function linq.from(arg)
    return linq.new(arg)
end

-- General constructor (`from`). Tries to guess the user's intentions.
function linq.new(source)
    local sourceLength = table.getn(source)
    ...
    error("Unsupported source with type '"..sourceType.."'!", 2)
end
    ...
    local function factory()
        ...
        return function()
            ...
            while index ~= nil do
                ...
                if table.getn(currentBatch) < size then
                    value, index = it()
                else
                    break
                end
            ...
            if index == nil then
                ...
                if table.getn(currentBatch) == 0 then
                    return nil, nil
                end
            end
